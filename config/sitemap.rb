...
SitemapGenerator::Sitemap.add_links do
  ...
  Spree::Post.live.order("posted_at DESC").each do |post|
    add full_post_path(post.blog,post.path), :lastmod => post.posted_at
  end
  Spree::Schwein.order("date_of_death DESC").each do |schwein|
    add schwein_path(schwein.permalink), :lastmod => schwein.updated_at
  end
  Spree::Rind.order("date_of_death DESC").each do |rind|
    add rind_path(rind.permalink), :lastmod => rind.updated_at
  end
